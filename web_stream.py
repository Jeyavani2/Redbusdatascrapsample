...
if selected=="Home":
   ...
   # css_file = os.path.join("Downloads", "styles.css")
...
# Read the CSS content
  ...
  #  st.markdown(f"<style>{css}</style>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        query_distinct_state="select distinct state_name from route;"
        ...
        filter_routes=f"select route_name from route where state_name = '{state}';"
    ...
    with col2:
        ...
        query_bus_type="select distinct bus_type from bus;"
        ...
        price=st.slider("Price Range", min_value=0, max_value=3000, value=(0, 3000), step=100)
